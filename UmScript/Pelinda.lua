...
local ServiceID = string.char(97, 115, 104, 98, 111, 114, 110, 110, 104, 117, 98)
...
local AshDirect = string.char(65, 115, 104, 98, 111, 114, 110, 110, 72, 117, 98, 47, 115, 97, 118, 101, 100, 95, 107, 101, 121, 46, 116, 120, 116)


PandaAuth:Initialize({
    Service = "ashbornnhub",
    API_Key = "s2RgXVn0obOGpk8J",
    DisplayName = "AshbornnHub",
    IsDebug = true,
    Allow_BlacklistUsers = false,
    GUIVersion = false,
    EnableWebhook = false,
    Webhook_URL = "https://discord.com/api/webhooks/1248263867897741312/XwmrB0DGtN4jIYvkJqliRxrp82i-Pj17lPJCHxOc-2ZCiigspIjt6mGEK2X-vjKjaOC1",
    Authenticated = function() print("[AshbornnHub] Key authenticated successfully! Access granted.") end,
    NotAuthenticated = function() print("[AshbornnHub] Key authentication failed. Access denied.") end
})
   local function generateRandomKey()
       local randomString = HttpService:GenerateGUID(false)
       randomString = randomString:gsub("-", "")
       return "AshbornnHub_" .. randomString
   end
...
function validateKey(keyText)
    if PandaAuth:ValidateKey(generateRandomKey()) then
        -- Bypass Detected
       elseif PandaAuth:ValidateKey(keyText) then
        loadScript()
        destroyUI()
       else
        SendNotif("AshbornnHub", "Invalid Key", 3)
