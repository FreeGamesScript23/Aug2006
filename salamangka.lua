...
local Fluent = loadstring(game:HttpGet("https://raw.githubusercontent.com/FreeGamesScript23/Aug2006/refs/heads/main/FluentUI-Modified.lua"))()
    ...
    if rootPart then
        ...
        if teleportMethod == "Instant" then
            ...
            repeat
                ...
                local speed = distance < slowDownDistance and math.clamp(distance * 1, 50, moveSpeed) or moveSpeed
                rootPart.CFrame = CFrame.new(rootPart.Position + (targetPosition - rootPart.Position).Unit * speed * game:GetService("RunService").Heartbeat:Wait())
            until distance <= arrivalThreshold
        end
        task.wait(0.1)
        isTeleporting = false
    end
...
local ZoneFishing, renderSteppedConnection = false, nil
...
ZoneFishingToggle = Tabs.Fish:AddToggle("ZoneFishing", {
    Title = "Enable Zone Casting",
    Default = false,
})

OverrideZoneToggle = Tabs.Fish:AddToggle("EnableOverrideZone", {
    Title = "Enable Override Zone",
    Default = false,
})

OverrideZoneToggle:OnChanged(function(value)
    isOverridingZone = value
end)

local function updateSelectedZone()
    local directionOffset = (Direction == "Up") and Ycoord or -Ycoord
    local targetZone

    if OverrideZoneToggle.Value and overrideZoneName then
...
local function refreshWaterZoneNames()
    ...
    table.sort(WaterZoneNames)  -- Sort the WaterZoneNames alphabetically
    Dropdown:SetValues(WaterZoneNames)
    OverrideZoneDropdown:SetValues(OverrideZoneValues)

    if OverrideZoneToggle.Value and overrideZoneName and workspace.zones.fishing:FindFirstChild(overrideZoneName) then
        updateSelectedZone()
    else
        updateSelectedZone() 
    end
...
ZoneFishingToggle:OnChanged(function(value)
    ...
    if value then
        ...
        renderSteppedConnection = game:GetService("RunService").RenderStepped:Connect(function()
            local playerTool = game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChildWhichIsA("Tool")
            if playerTool then
...
OverrideZoneToggle:OnChanged(function()
    updateSelectedZone()
end)
...
local Locations = {
    ...
    ["Archeological Site"] = CFrame.new(4015, 131, 74),
...
local subLocations = {
    ["Archaeologists Boat"] = CFrame.new(1175, 132, 2459),
...
local NPCLocations = {
    ...
    ["Dr. Finneus"] = workspace.world.npcs:FindFirstChild("Dr. Finneus") and workspace.world.npcs["Dr. Finneus"].HumanoidRootPart.CFrame or CFrame.new(1172, 132, 2457),
...
local fluentUI = "ScreenGui"

coreGui.ChildRemoved:Connect(function(removedChild)
    if removedChild.Name == fluentUI then
